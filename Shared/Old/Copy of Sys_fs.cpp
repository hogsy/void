...
{
public:
	CSearchPath()  { path =0; next=prev = 0; }
...
bool CFileManager::AddSearchPath(const char * path, const int pathlen)
{
	...
	if(!BuildDirectoryList(m_lastpath->path,"/"))
	{
...
===========================================
*/
bool CFileManager::BuildDirectoryList(const char *basepath,
									  const char *dirname)	//Proposed name of the directory
{
	...
	sprintf(dirpath,"%s/%s%s*.*",m_exepath,basepath,dirname);
	...
	char itemname[COM_MAXPATH];

	if(numfiles)
	{
		int			hash = 0;
		iterator = &filelist;
		CFileList * iterfiles = 0;
		
		for(i=0;i<numfiles;i++)
		{
			hash = (int)iterator->string[0];
						
			strcpy(itemname ,dirname);
			strcat(itemname, iterator->string);

			//if there are items, loop ahead until we are at the end;
			if(m_filetable[hash].file)
			{
				iterfiles = &m_filetable[hash];
				while(iterfiles->next)
				{
					//If there is already a file by this name here
					//If will only be
					if(strcmp(iterfiles->file->filename, itemname) == 0)
					{
					}
					iterfiles = iterfiles->next;
				}
				iterfiles->next = new CFileList();
				iterfiles = iterfiles->next;
				iterfiles->file = new CFile(itemname,basepath);
			}
			else
			{
				m_filetable[hash].file = new CFile(itemname,basepath);
			}
	...
	if(numdirs)
	{
		...
		//Recurse to add contents of each dir
		for(i=0;i<numdirs;i++)
		{
			strcpy(itemname,dirname);
			strcat(itemname,iterator->string);
			...
			BuildDirectoryList(basepath,itemname);
			iterator = iterator->next;
		}
	}
	return true;
...
{
	CFileList * iterator = 0;

	for(int i=0;i<FS_NUMHASHES;i++)
	{
		if(m_filetable[i].file)
		{
			iterator = &m_filetable[i];
			while(iterator->next)
			{
