...
void CEntMover::SetWorld(CWorld * pWorld)
{	m_pWorld = pWorld;
}


void CEntMover::NoClipMove(BaseEntity *ent, vector_t &dir, float time)
{	VectorMA(&ent->origin, time, &dir, &ent->origin);
}


void CEntMover::ClientMove(BaseEntity *ent, vector_t &dir, float time)
{
	// all the normals of the planes we've hit
	vector_t	hitplanes[MAX_CLIP_PLANES];	
	int			hits=0;				// number of planes we've hit, and number we're touching
